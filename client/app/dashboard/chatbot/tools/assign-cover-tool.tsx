...
interface AssignCoverToolResult {
    client_name: string
    date: string
    time: string
    status: string
...
type ExtendedToolInvocation = ToolInvocation & {
    result?: AssignCoverToolResult
}

export function AssignCoverTool(toolInvocation: ExtendedToolInvocation) {
    const [error, setError] = useState<string | null>(null)
    const [isLoading, setIsLoading] = useState(false)
    const { toast } = useToast()

    const assignCover = async (data: { client_name: string; date: string; time: string }) => {
        try {
            // Simulate API call
            ...
            toast({
                title: "Success",
                description: "Cover assigned successfully",
                variant: "default",
            })
            ...
            const errorMessage = error?.message || "Failed to assign cover. Please try again."
            toast({
                title: "Error",
    ...
    useEffect(() => {
        ...
        if (toolInvocation.state === "result" && !isLoading) {
            const { client_name, date, time } = toolInvocation.result;
            setIsLoading(true);
            ...
            assignCover({
                ...
                if (isMounted) {
                    setError(err.message || "Failed to assign cover");
                }
            });
        }
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-[320px]">
                <Card className="bg-background p-4 rounded-lg shadow-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <div className="font-medium">Assigning cover</div>
    ...
    if (toolInvocation.state === "result") {
        const { client_name, date, time, status } = toolInvocation.result
                ...
                <Card className="bg-background rounded-lg shadow-sm overflow-hidden">
                    ...
                    <div className="bg-muted p-3">
                        ...
                        >
                            ...
                            <div>
                                <div className="font-medium text-black text-sm">Cover Assigned</div>
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    <Label className="text-xs text-neutral-500 shadow-none">Date</Label>
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    <Label className="text-xs text-neutral-500 shadow-none">Time</Label>
