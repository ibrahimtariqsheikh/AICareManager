...
import { Loader2, CheckCircle2, BarChart, Calendar, DollarSign } from "lucide-react"
...
interface RevenueReportToolResult {
    period: string
    revenue: string
    status: string
}
...
type ExtendedToolInvocation = ToolInvocation & {
    result?: RevenueReportToolResult
}

export function RevenueReportTool(toolInvocation: ExtendedToolInvocation) {
    const [error, setError] = useState<string | null>(null)
    const [isLoading, setIsLoading] = useState(false)
    const { toast } = useToast()

    const generateRevenueReport = async (data: { period: string }) => {
        try {
            // Simulate API call
            ...
            return { ...data, revenue: "£25,000", status: 'generated' }
        } catch (error: any) {
            console.error("Error generating revenue report:", error)
            const errorMessage = error?.message || "Failed to generate revenue report. Please try again."
            toast({
                title: "Error",
    ...
    useEffect(() => {
        ...
        if (toolInvocation.state === "result" && !isLoading) {
            const { period } = toolInvocation.result;
            setIsLoading(true);
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-[320px]">
                <Card className="bg-background p-4 rounded-lg shadow-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <div className="font-medium">Generating revenue report</div>
    ...
    if (toolInvocation.state === "result") {
        const { period, revenue, status } = toolInvocation.result
                ...
                <Card className="bg-background rounded-lg shadow-sm overflow-hidden">
                    ...
                    <div className="bg-muted p-3">
                        ...
                        >
                            ...
                            <div>
                                <div className="font-medium text-black text-sm">Revenue Report Generated</div>
                    ...
                    <div className="p-3 space-y-3">
                        ...
                        >
                            ...
                            <div>
                                ...
                                <div className="text-[10px] text-neutral-700">Period</div>
                            </div>
                        </motion.div>
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    <Label className="text-xs text-neutral-500 shadow-none">Revenue</Label>
